...
import os
" Topo này có giới hạn Bandwith để số liệu metrics ổn định hơn sau mỗi lần đo"
# =======================================
...
class StaticRoutingTopo(Topo):
    def build(self):
        ...
        # ===== Link Configuration (with bandwidth) =====
        link_router_bw = 100  # Mbps 
        link_host_bw = 10     # Mbps 

        # Gateway <-> Routers
        self.addLink(gw, r1, intfName1='gw-eth1', params1={'ip': '10.0.10.1/24'}, bw=link_router_bw) 
        self.addLink(gw, r2, intfName1='gw-eth2', params1={'ip': '10.0.20.1/24'}, bw=link_router_bw) 
        self.addLink(gw, r3, intfName1='gw-eth3', params1={'ip': '10.0.30.1/24'}, bw=link_router_bw) 
        
        # Routers <-> Switches
        self.addLink(r1, s1, intfName1='r1-eth1', params1={'ip': '10.0.1.254/24'}, bw=link_router_bw)
        self.addLink(r2, s2, intfName1='r2-eth1', params1={'ip': '10.0.2.1/24'}, bw=link_router_bw)
        self.addLink(r2, s3, intfName1='r2-eth2', params1={'ip': '10.0.3.1/24'}, bw=link_router_bw)
        self.addLink(r3, s4, intfName1='r3-eth1', params1={'ip': '10.0.4.1/24'}, bw=link_router_bw)

        # Gateway <-> Cloud
        self.addLink(gw, cloud, intfName1='gw-eth4', params1={'ip': '10.0.100.1/24'}, bw=link_router_bw)

        # Switch <-> Hosts
        for h in hosts_s1:
            self.addLink(s1, h, bw=link_host_bw)
        for h in hosts_s2:
            self.addLink(s2, h, bw=link_host_bw)
        for h in hosts_s3:
            self.addLink(s3, h, bw=link_host_bw)
        for h in hosts_s4:
            self.addLink(s4, h, bw=link_host_bw)
...
def run():
    ...
    # Router 1
    r1.cmd('ip route add default via 10.0.10.1')
    ...
    # Router 2
    r2.cmd('ip route add default via 10.0.20.1')
    r2.cmd('ip route add 10.0.0.0/24 via 10.0.20.1')
    r2.cmd('ip route add 10.0.4.0/24 via 10.0.20.1')
    r2.cmd('ip route add 10.0.100.0/24 via 10.0.20.1')

    # Router 3
    r3.cmd('ip route add default via 10.0.30.1')
    r3.cmd('ip route add 10.0.0.0/24 via 10.0.30.1')
